...
class Producto(ObjetoAdministracion):
	def __init__(self,nombrep,preciop,cantidad,id_dist):
	...
	def modifyDatos(self,linea):
		...
		cadena =id_pro+" "+self.nombrep + " " + self.preciop + " " + self.cantidad+" "+self.id_dist
		return cadena
##**------------------------------------------------------		
class Cliente(ObjetoAdministracion):

	def __init__(self,nombrec,rfc):
	...
	def modifyDatos(self,linea):
		...
		while 1:
			...
			if dato.lower()=='n':
				...
				print("Opción no válida")
		cadena=id_cliente+" "+self.nombrec+" "+self.rfc 
		return cadena
##**------------------------------------------------------
class Distribuidor(ObjetoAdministracion):
	def __init__(self,nombred,rfc):
	...
	def existInFile(self):
		try:
			count=0
			with open("distribuidores",'r') as db:
				for linea in db:
					count=count+1
	...
	def modifyDatos(self,linea):
		...
		while 1:
			...
			if dato.lower()=='n':
				...
				print("Opción no válida")
		cadena=id_dist+" "+self.nombred+" "+self.rfc 
		return cadena
##**------------------------------------------------------

class Usuario(ObjetoAdministracion):
	def __init__(self,nombre, contrasena, perm):
		self.nombre=nombre
	...
	def menu_administracion(self):
		while 1:
			...
			try:
				...
				print("ERROR: No existe el archivo de "+self.objeto_name+"s")
...
def login():
	while 1:
		perm=checa_permisos()[:-1]
		if perm == "admin":
			presenta_interfaz_admin()
		elif perm == "mortal":
